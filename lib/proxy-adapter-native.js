...
export default class ProxyAdapterNative extends ProxyAdapterDom {
  ...
  rerenderNativePage() {
    ...
    if (pageRenavigate) {
      const frame = oldPageElement.nativeView.frame;
      if (!frame) {
      ...
      const removeChild = child => oldPageElement.removeChild(child);
      oldPageElement.childNodes.forEach(removeChild);
      this.createComponent(oldPageElement, null);
    }
  }
