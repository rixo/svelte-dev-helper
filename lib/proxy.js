    ...
    _register(options) {
      ...
      try {
        ...
        if (!rb) {
          console.error(e);
          console.warn(
            'Full reload required. Please fix component errors and reload the whole page'
          );
          return;
        }
        ...
        console.warn(e);
        console.warn(
          this._debugName + ' could not be hot-loaded because it has an error'
        );

        //resolve to previous working version of component
        this.proxyTarget = new rb(options);
        console.info(
          '%c' + this._debugName + ' rolled back to previous working version',
          'color:green'
        );
      ...
      let methods = Object.getOwnPropertyNames(
        Object.getPrototypeOf(self.proxyTarget)
      );
    ...
    _rerender() {
      ...
      if (mountpoint && isMounted) {
        ...
        if (
          ...
          this.proxyTarget._fragment &&
            this.proxyTarget._fragment.p(changed, oldstate);
