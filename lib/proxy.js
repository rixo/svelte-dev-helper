...
const forwardedMethods = ['$set', '$on', '$capture_state', '$inject_state'];
const $$_keys = [
  'ctx',
  // state
  'props',
...
//         -> cmp.m
//
// For now, we ~~only~~ use the m hook, and we call it ourselve if we see
// that the component has been mounted at creation time.
//
// Now we also need the d hook (because $destroy is not called in child
...
const captureState = cmp => {
  ...
  const restore = { callbacks, bound };
  if (cmp.$capture_state) {
    restore.state = cmp.$capture_state();
  } else {
    restore.props = ctx;
  }
  return restore;
...
const restoreState = (cmp, restore) => {
  ...
  const { state, callbacks, bound } = restore;
  if (callbacks) {
  ...
  // DEBUG how could $inject_state not be here if we've got a state
  //   from $capture_state? if really that can happen, we should
  //   crash HMR (decline) at this point
...
export const initProxy = (Adapter = DomAdapter) =>
  function createProxy(id, component, hotOptions) {
    ...
    const reload = ({ component, hotOptions }) => {
      ...
      resetErrors();
      instances.reduce(
        (success, rerender) => success && rerender(),
        true
      );
      if (!lastErrors) {
