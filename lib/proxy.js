...
import { configure, getConfig, isNative, observeConfig } from './config';
...
// apply monkey patch when native options is enabled
//
// can't do this before because we would require a TNS module that is not
// available in non-native projects
//
observeConfig(({ native }) => {
  if (native && module.hot) {
    const { patchShowModal } = require('./patch-page-show-modal');
    patchShowModal();
  }
});
...
export function createProxy(id) {
  const Adapter = isNative()
    ? require('./proxy-adapter-native').default
    : require('./proxy-adapter-dom').default;

  class ModuleProxyComponent extends ProxyComponent {
    constructor(options) {
      super(Adapter, id, options);
    }
  }
