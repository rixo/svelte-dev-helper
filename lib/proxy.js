...
const $$_keys = [
  ...
  'on_mount',
  'on_destroy',
  // before_render/after_render: svelte <= 3.5
  'before_render',
  'after_render',
  // before_update/after_update: svelte <= 3.6
  'before_update',
  'after_update',
  'context',
  // everything else
...
export const initProxy = (Adapter = DomAdapter) =>
  function createProxy(id, component, hotOptions) {
    ...
    const reload = ({ component, hotOptions }) => {
      ...
      resetErrors();
      instances.reduce((success, rerender) => success && rerender(), true);
      if (!lastErrors) {
